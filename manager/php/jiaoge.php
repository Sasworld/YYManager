...
//			$paidanid = "PD514182";
	...
	$uid = $managerid; // connSql中设置
	$trademoney = $array_data["trademoney"];
	$paidanid = $array_data["paidanid"];
	
	$sql = "select pduid,needamount,version from paidan where pdid='$paidanid'";
	$res = $conn->query($sql);
	if ($res->num_rows > 0) {
	    $row = $res->fetch_assoc();
		$traderid=$row["pduid"];
		$arr = jiaoge($conn, $trademoney, $paidanid, $uid, $traderid);
	}else{
		$arr = array(
			'result'=>"账户查询失败", 
		);
	...
	//交割
	function jiaoge($conn, $trademoney, $paidanid, $uid, $traderid){
			// 查看需求总额
